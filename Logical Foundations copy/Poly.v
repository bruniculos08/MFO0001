...
(* 
Anotação da aula -> existem 3 tipos de Polimorfismo
  -> com restrições ou de sobrecarga (ad-hoc, do latim...
     ...'para este caso') (ex.: somatório de lista em haskell);
  -> subtipagem ou inclusão (herança de classe);
  -> paramétrico ou universal (o tipo é um parâmetro)
*)
...
Inductive list (X:Type) : Type :=
  ...
  (* Note q to do construtor recebe o argumento X na frente do
     Pattern match; ex.:
     cons nat 3 (cons nat 2 (nil nat)) *)
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intro. intro. intro. intros. induction l as [|h t IH].
  (* O próximo elemento (na hipótese do passo da indução) 
   é um elemento com cabeça h e cauda t, e a hipótese é sobre t.
   Obs.: note que temos que dar intro no tipo pois ele também é
   um variável (e há um quantificador sobre ele). *)
  - simpl. reflexivity.
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  (* Lembre-se que uma função involutiva é uma função igual a
   própria inversa *)
  rev (rev l) = l.
Proof.
...
Definition compose {X Y Z : Type} (g : Y -> Z) (f : X -> Y) (x : X) := 
  g (f x).
...
(* Lista de anotações importantes (Bruno R. dos Santos): 
  
  (1) note que '*' é um construtor de tipos enquanto '( , )' é um operador
  de dados - ambos server para indicar tuplas (duplas) porém um indica
  tipo e outro constroi dado. 
  
  (2) o fun é semelhante ao lambda em Python e portanto é muito útil para
  descrever funções (anônimas, isto é, sem nome declarado) simples. 
  *)
